...
//	m_pShader[SHADER_HIT_EFFECT]	= GET_SINGLE(CShaderManager)->FindShaderNonCount(HIT_EFFECT_SHADER);
//
...
//	m_pTarget[TARGET_HITEFFECT]		= GET_SINGLE(CViewManager)->FindRenderTarget("HitEffect");
//
...
//	for (int j = 0; j < m_tRenderObj[RG_NORMAL].iSize; ++j)
//	{
//		if(m_tRenderObj[RG_NORMAL].pList[j]->IsShaodw())
...
bool CRenderManager::Init()
{
	...
	m_tFinalCBuffer.iDistortion = 0;
	m_tFinalCBuffer.fWaveScale = 0.0f;
...
void CRenderManager::FindMagicNumber(float fTime)
{
	if (m_bMagic == true)
	{
		...
		{
			EnableFilter(CFT_HDR);
			ImGui::SliderFloat("MiddleGrey", &m_fMiddleGrey, 0.0f, 6.0f);
			ImGui::Text("MiddleGrey : %f", m_fMiddleGrey);
			ImGui::SliderFloat("LumWhite", &m_fLumWhite, 0.0f, 50.0f);
			ImGui::Text("LumWhite : %f", m_fLumWhite);
		...
		{
			EnableFilter(CFT_BLUR);
			ImGui::Checkbox("DepthOFField", &m_bDOF);

			if (m_bDOF)
			{
				((CCSBlur*)m_pFilter[CFT_BLUR])->EnableDOF();
				ImGui::SliderFloat("DOFEnd", &m_fDofEnd, 25.f, 1000.0f);
				ImGui::Text("DOFEnd : %f", m_fDofEnd);
				((CCSBlur*)m_pFilter[CFT_BLUR])->SetFar(m_fDofEnd);
			}
			else
			{
				((CCSBlur*)m_pFilter[CFT_BLUR])->DisableDOF();
				ImGui::SliderFloat("BlurDensity", &m_fBlurDensity, 0.f, 1.0f);
				ImGui::Text("BlurDensity : %f", m_fBlurDensity);
				((CCSBlur*)m_pFilter[CFT_BLUR])->SetDensity(m_fBlurDensity);
			}
			ImGui::SliderInt("NumSample", &m_iNumPixel, 3.f, 41.0f);
			ImGui::Text("NumSample : %f", m_iNumPixel);
			((CCSBlur*)m_pFilter[CFT_BLUR])->SetNumPixel(m_iNumPixel);
		}
		else if (m_bBlur == false && m_bBlurOn == false)
			DisableFilter(CFT_BLUR);
...
void CRenderManager::SetMotionBlur(bool bEnable, int iMotionBlur)
{
	if (bEnable)
	{
...
void CRenderManager::SetSkyEnable(bool bEnable)
{
	m_pSkyObj->SetEnable(bEnable);
}

void CRenderManager::EnableDistortion()
{
	m_bDistortion = true;
	m_bBlur = true;
	m_fBlurDensity = 0.0f;
	m_fTime = 0.0f;
	m_fWaveLimitTime = 2.5f;
	m_tFinalCBuffer.iDistortion = 1;
	m_tFinalCBuffer.fWaveSpeed = 100.0f;
	m_tFinalCBuffer.fWaveScale = 0.5f;
	m_tFinalCBuffer.vWaveCenter.x = 0.5f;
	m_tFinalCBuffer.vWaveCenter.y = 0.5f;
}

void CRenderManager::Distortion(float fTime)
{
	if (m_bDistortion)
	{
		m_fTime += fTime;

		m_fBlurDensity = sin(DegreeToRadian(m_fTime * (1 / m_fWaveLimitTime) * 180.0f));

		m_tFinalCBuffer.fWaveScale -= m_tFinalCBuffer.fWaveScale * m_fWaveLimitTime * fTime;
		
		if (m_fTime > m_fWaveLimitTime)
		{
			m_fTime = 0.0f;
			m_tFinalCBuffer.iDistortion = 0;
			m_tFinalCBuffer.fWaveScale = 0.0f;
